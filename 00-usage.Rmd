...
1. Adopt the course in its entirety:
  ...
  1. Clone the project from GitHub to RStudio (in RStudio, File > New Project > Version Control > Git)
    1. Install the  `renv` package and install all packages within the project using `renv::restore()`
  1. If you wish to build the book at any point, practicals (RMarkdown files) without data can be removed by commenting out the file (with a #) in the `_bookdown.yml` file
...
If raising an issue provide sufficient detail that it can be reproduced with a minimal worked example (where possible). Be sure to include the following:
...
This is version 3.0 of the practical book

Previous versions can be found at the following links.

2020-2021 academic year: https://andrewmaclachlan.github.io/CASA0005repo_20202021/
