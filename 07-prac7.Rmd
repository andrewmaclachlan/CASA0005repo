...
The `style` argument means the style of the output --- `B` is binary encoding listing them as neighbours or not, `C` globally standardised ()
...
The **lag** model is where the value of the dependent might be influenced by neighboring dependent (yes dependent) variables being incorporated into the model with a spatial weight matrix. It is saying that GCSE scores in one area might be influenced by GCSE scores in another. You might run a lag model if you identify spatial autocorrelation (closer spatial units have similar values) with Moran's I.

The **error** model deals with spatial autocorrelation (closer spatial units have similar values) of the residuals (vertical distance between your point and line of model) again, shown here through Moran's I. Here **we do not** assume that neighboring independent variables are influencing the dependent variable but through an issue with the model or data (e.g. it's from some independent variable not included) - it's a **problem** we include in the model. For example, GCSE scores are may be similar in near London Boroughs but **not because many** students cross Borough boundaries because students (or families where students usually have higher scores) cluster. There is no spatial process (no cross Borough interaction) just a cluster of the variable of interest. 

Usually you might run an **lag** model when you have an idea of what is causing the spatial autocorrelation and an **error** model when you aren't sure. However, you can also use a more scientific method - the Lagrange Multiplier test.
...
* Is it a global model (error / lag) or a local model (G
