...
* Scale vs Location: flat and horizontal line. This is the homoscedasticity (errors/residuals in the model exhibit constant / homogeneous variance)

* Residuals vs Leverage - Identifies outliers (or influenttial observations), the three largest outliers are identified with values in the plot.
...
* Likelihood ratio (LR) test shows if the addition of the lag is an improvement and if that's significant. This code would give the same output...
...
* Wald test (often not used in interpretation of lag models), it tests if the new parameters (the lag) should be includes it in the model...if significant then the new variable improves the model fit and needs to be included. This is similar to the LR test and i've not seen a situation where one is significant and the other not. Probably why it's not used!
