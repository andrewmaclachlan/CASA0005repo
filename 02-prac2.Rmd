...
2.  Select Start a project in a brand new working directory and create a new project in a directory of a new ‘wk2’ directory on your N: drive:
...
2.  The R Script should open up on the top-left of your GUI. **From now on type everything in this R script file and save it**
...
2.  We can now begin to write a script without the need to run each line every time we press enter. In the script editor type:
...
2.  Is it a single row (e.g. vector)
3.  Is there an extra dimension (e.g. matrix or array)
4.  Are there many different types combined (e.g list)
...
2.  However, we could also again use a more 'data sciency way'...selecting the columns that contain certain words ...
...
2.  Round the whole table based on if the column is numeric (this isn't required and we're not adding a new column). Here we are using `across` that applies some kind of transformation across the columns selected (or that are numeric in this case).
...
::: {.infobox .warning data-latex="{note}"}
Week 5 is dedicated to mapping where we will explore this is much more detail. This is just a breif introduction to mapping.
:::

`tmap` follows the idea of the grammar of graphics, similar to `dplyr` being the grammar of data manipulation. With the grammar of graphics the main data being mapped defines the location somewhere on Earth (loaded with `tm_shape`). Map layers are then added (e.g. `tm_polygons`, `tm_symbols()` for points, lines, `tm_raster`). These layers can then be styled or "filled" with a column of data (and more!) (e.g.  `tm_polygons(fill = "rate_of_job_seekers_allowance_jsa_claimants_2015"`)

So to conclude, data first then `+` layer with arguments controlling the display.
...
1.  We can create a choropleth map very quickly now using `qtm()`
...
Now we have the basemap, set `tmap` to plot, add the basemap, add the shape (our London layer), tell it which attribute to map (job seekers), the style to make the colour divisions, the transparency (alpha), compass, scale and legend.

But remember what i explained earlier... data first then `+` layer with aesthetics controlled through the arguments. 
...
See the [leaflet extras page](https://leaflet-extras.github.io/leaflet-providers/preview/) for additional options
