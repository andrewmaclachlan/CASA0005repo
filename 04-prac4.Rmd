...
To start Git you **need to be in a RStudio project**. Instructions below show you how to do this in various scenarios. In the first instance we just copy it from the remote (GitHub) - so don't do this now....but a very simple way is to again load the `usethis` package in the console and the function `use_git()`, then type option 1. 
